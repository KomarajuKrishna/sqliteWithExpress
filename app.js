...
const app = express();
// const moviesRouter = require("./routes/movieRouters");
...
const initializeDBAndServer = async () => {
  try {
    ...
    app.listen(4897, () => {
      console.log("Server is Running at: localhost:3009");
    });
  } catch (error) {
...
app.get("/movies", verifyAccessToken, async (request, response) => {
  try {
...
app.post("/movies", verifyAccessToken, async (request, response) => {
...
app.put("/movies/:movieId", verifyIDInDB, async (request, response) => {
  const { movieId } = request.params;
  const { movie_name, release_year, duration } = request.body;
...
app.delete("/movies/:movieId", verifyIDInDB, async (request, response) => {
  const { movieId } = request.params;
  try {
